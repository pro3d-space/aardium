...
function showReportDialog(parent, type, config) {
  ...
  const systemInfo =
    `OS: ${activeReport.os}\n` +
    `CPU: ${activeReport.cpu}\n` +
    `GPU: ${activeReport.gpu}\n` +
  ...
  // On Linux, modal windows can freeze the whole UI: https://github.com/electron/electron/issues/21128
  // On macOS, traffic light buttons are hidden for modal windows leaving us without a way to close the window.
  let parentValid = parent && !parent.isDestroyed();
  const isModal = parentValid && process.platform !== 'linux' && process.platform !== 'darwin'
